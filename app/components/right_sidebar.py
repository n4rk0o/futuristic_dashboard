...
def system_time_section() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.p(DashboardState.current_date, class_name="text-sm text-gray-400 mb-4"),
        rx.el.div(
            rx.el.div(
                rx.el.p("Uptime", class_name="text-xs text-gray-500 mb-1"),
                rx.el.p(
            ...
            rx.el.div(
                rx.el.p("Time Zone", class_name="text-xs text-gray-500 mb-1"),
                rx.el.p(
...
def quick_action_button(action: QuickActionData) -> rx.Component:
    return rx.el.button(
        rx.icon(tag=action["icon"], class_name="size-5 mb-1 sm:mb-2"),
...
def quick_actions_section() -> rx.Component:
    return rx.el.div(
        ...
        rx.el.div(
            rx.foreach(DashboardState.quick_actions, quick_action_button),
            class_name="grid grid-cols-2 gap-2 sm:gap-3",
        ),
        class_name="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4 shadow-md backdrop-blur-sm mb-6",
    )
...
def resource_allocation_item(resource: ResourceAllocationData) -> rx.Component:
